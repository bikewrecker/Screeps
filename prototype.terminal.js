StructureTerminal.prototype.sellMinerals = function() {
    ...
    var minPrice = {'Z': 0.1, 'H': 0.15};
    
    if(this.store[RESOURCE_ENERGY] >= 2000 && this.store[mineral] > 10000) {
        if(mineral == 'H'){
            ...
            var orders = Game.market.getAllOrders(order => order.resourceType == mineral && order.type == ORDER_BUY && order.remainingAmount >= 200 && Game.market.calcTransactionCost(200, this.room.name, order.roomName) < 400);
            orders.sort(function(a,b){return b.price - a.price;});
            if(orders[0].price >= minPrice[mineral]) {
                let result = Game.market.deal(orders[0].id, 200, this.room.name);
                 if (result == 0) {
                     console.log(mineral + ' buy orders found: ' + orders.length);
                     console.log('Best price: ' + orders[0].price);
                    console.log("Order completed successfully for " + Game.market.calcTransactionCost(200, this.room.name, orders[0].roomName) + " energy. " +  orders[0].price * 200 + " total credits recieved");
                }
            }
        }
    }
